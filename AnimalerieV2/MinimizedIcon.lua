...
function MinimizedIcon:Constructor(image, width, height, callback)
		...
		if not self.thresholdTrigger then
			if (args.Button == Turbine.UI.MouseButton.Right) then
				...
				if(self.IsShiftKeyDown())then
					...
					SaveSettings();
				elseif(settings["isWindowVisible"]["value"] == false) then
					ReloadWindow();
					AnimalerieWindows:SetZOrder(AnimalerieWindows:GetZOrder()+1);
					SaveSettings();
				else
	...
	self.trigger.MouseMove = function( sender, args )
		if(settings["altEnable"]["value"] == false)then
			if self.drag then
				local dx = args.X - self.mx0;
				local dy = args.Y - self.my0;
				-- Turbine.Shell.notificationLine("dragged "..dx.." "..dy);
